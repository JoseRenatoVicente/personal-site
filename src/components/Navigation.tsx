...
const Navigation = ({ data, navClass }: NavigationProps) => {
  const renderLinks = () =>
    data?.map((navItem, i) => {
      ...
      return isExternal ? (
          <Link key={i} href={navItem.url} className={className} prefetch={false} target="_blank" rel="noopener noreferrer">
            {navItem.label}
          </Link>
        ) : (
          <Link key={i} href={navItem.url} className={className} prefetch={false}>
            {navItem.label}
          </Link>
        );
    })
